...
// Function to move specific draggables associated with the selected honeypot to the dropbox
function selectHoneypot(honeypotId) {
    const dropBox = document.getElementById("div1");
    dropBox.innerHTML = ''; // Clear previous content

    // Select the interaction-level draggable associated with the specified honeypot
    const interactionDraggable = document.querySelector(`.draggable[data-honeypot='${honeypotId}'][data-type='interaction']`);

    // If the interaction-level draggable exists, move it to the dropbox
    if (interactionDraggable) {
        dropBox.appendChild(interactionDraggable);
    } else {
        console.warn(`No interaction draggable found for honeypot: ${honeypotId}`);
    }
}