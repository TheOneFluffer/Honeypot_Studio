    ...
    try {
        ...
        for (const entry of filteredLogs) {
            ...
            await esClient.index({
                index: 'honeypot-logs', // Replace with your Elasticsearch index name
                body: {
                    honeypotName,
                    severity: entry.severity,
                    log: entry.log,
        ...
        res.status(200).json({
            message: `Logs for honeypot '${honeypotName}' processed one by one, saved to MongoDB, and sent to ELK stack successfully.`,
        });
    } catch (error) {
